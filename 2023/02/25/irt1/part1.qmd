...
options(knitr.svg.object = TRUE)  # Embed svg as raw XML
```
...
basic level of understanding, due to its conventional presentation and
...
![Simulation is _playing god_ in a small world. Similar to model fitting, but in _reverse_ direction.](tenet2.gif)
...
the same purpose (e.g., the **cumulative distribution function** for the normal
distribution).
...
We have got all we need, let's construct the remaining columns to complete this
simulation. I will compute the probabilities (`P`) from column `Mu`. Column `P`
could then generate column `R`, the item responses, through the Bernoulli
distribution.
...
be broken somehow, and instead generates the ones with, e.g., $P=0.53$.

The [law of large numbers][large-num] can help us here. Since `rbern()`
generates one with probability $P$, if we run `rbern()` many times, the
proportion of one in the outcomes should converge to $P$. The second argument
`n` in `rbern()` is a handy way to achieve this. You can increase `n` to see if
the result would come closer to $0.5$.
...
A more general way to rerun chunks of code is through the for loop or convenient
wrappers such as the `replicate()` function.
...
what? **_Simulation_** is the answer. 
...
several times, each with different parameter values set. If the parameters are
always recovered by the statistical model, we can then be confident that the
model is properly constructed and correctly coded. So **simulation is really not
an option when doing statistics**. It is the only safety that helps us guard
against bugs in our statistical models, both programmatical and theoretical
ones. Without first testing the statistical model on simulated data, any
