...
parameters{
  ...
  vector[Nw] zW;
  vector[Nw] muW;
  vector[2] bOjR;  // main effect of judge origin on score
  vector[2] bOwR;  // main effect of wine origin on score
...
model{
  ...
  bOjR ~ normal( 0, 1 );
  bOwR ~ normal( 0, 1 );
  to_vector( Int ) ~ normal( 0, 1.5 );
  for ( i in 1:N ) {
    phi[i] = W[Wid[i]] + J[Jid[i]] + bOjR[Oj[i]] + bOwR[Ow[i]] + Int[Oj[i], Ow[i]];
  }
